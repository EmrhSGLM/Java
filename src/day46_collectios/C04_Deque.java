package day46_collectios;

import java.util.Deque;
import java.util.LinkedList;

public class C04_Deque {

	public static void main(String[] args) {
		
		Deque<String> ciftBasli = new LinkedList<>();
		
		ciftBasli.add("Yildiz");
		ciftBasli.add("Ali");
		System.out.println(ciftBasli); // [Yildiz, Ali] add() Queu'dan geldiginden sona ekler
		
		ciftBasli.addLast("Emine");
		System.out.println(ciftBasli); // [Yildiz, Ali, Emine] Deque'den gelen ozellik
		
		System.out.println(ciftBasli.pop()); // Yildiz ==> Ilk elementi sildi ve bize dondurdu 
											//  pop ilk elementi siler ve delil olarak bize getirir
		
		System.out.println(ciftBasli); // [Ali, Emine]
		
		ciftBasli.push("Seyfullah"); // Basa ekleme yapti
		System.out.println(ciftBasli); // [Seyfullah, Ali, Emine]
		
		ciftBasli.add(null);
		System.out.println(ciftBasli); // [Seyfullah, Ali, Emine, null]
		

	}

}
